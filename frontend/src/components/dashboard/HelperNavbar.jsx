  ...
  return (
    <header className="sticky top-4 z-50 px-4 lg:px-6 transition-all duration-300">
      <div className="mx-auto max-w-[1200px] h-[70px] rounded-full border border-cr-border px-4 md:px-5 lg:px-6 flex items-center justify-between bg-cr-card/80 backdrop-blur-[16px] shadow-[0_15px_40px_rgba(0,0,0,0.12)] transition-colors duration-300">
          {/* Logo */}
          <Link to="/dashboard/helper" className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-full bg-cr-primary flex items-center justify-center shadow-md">
              <HeartHandshake size={22} className="text-white" />
            </div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-cr-text-primary">
                CareRide AI
              </h1>
              <span className="px-2 py-0.5 text-[11px] rounded-full bg-cr-sage/20 text-cr-primary border border-cr-border">
                Helper
              </span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-6">
            <ThemeToggle />
            <div className="flex items-center gap-2 text-cr-text-muted text-sm">
              <User size={15} />
              <span>{user?.email || "helper@gmail.com"}</span>
            </div>
            <button
              onClick={() => setShowSignOutModal(true)}
              className="flex items-center gap-2 text-cr-text-muted hover:text-cr-primary transition font-medium"
            >
              <LogOut size={17} />
              Sign Out
            </button>
