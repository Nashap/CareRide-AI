...
function RiderSidebar() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('toggle-rider-sidebar', handleToggle);
    return () => window.removeEventListener('toggle-rider-sidebar', handleToggle);
  }, []);
    ...
    <div className="bg-cr-card md:rounded-[24px] border-r md:border border-cr-border md:shadow-lg overflow-y-auto h-full flex flex-col pt-4 md:pt-0">
      <div className="md:hidden flex items-center justify-between p-6 mb-2">
        <div className="flex items-center gap-3">
          ...
          <span className="text-lg font-bold text-cr-text-primary tracking-tight">Rider Menu</span>
        </div>
        <button 
